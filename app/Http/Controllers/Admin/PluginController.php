...
class PluginController extends Controller
{
    ...
    public function upload(Request $request)
    {
        ...
        // Extract folder
        $extract_asset = $zipper->make($request->file('theme-zip'))->folder('asset')->extractMatchingRegex(public_path('themes/'.strtolower($theme_name[0])), '/\.jpg$|\.svg$|\.png$|\.gif$|\.jpeg$|\.css$|\.font$|\.js$|\.scss$|\.ttf$|\.woff$|\.woff2$|.eot$/i');
        ...
        $extract_template = $zipper->make($request->file('theme-zip'))->folder('asset')->extractMatchingRegex(storage_path('themes/'.strtolower($theme_name[0]).'/asset'), '/\.jpg$|\.svg$|\.png$|\.gif$|\.jpeg$|\.css$|\.font$|\.js$|\.scss$|\.ttf$|\.woff$|\.woff2$|.eot$/i');

        $store = new Plugin();
        $store->name = strtolower($theme_name[0]);
        $store->type = 'theme';
        $store->thumbnail = '/themes/'.strtolower($theme_name[0]).'/thumbnail.jpg';
        $store->save();
