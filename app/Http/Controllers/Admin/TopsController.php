...
class TopsController extends Controller
{
    ...
    {
        $url_posters = Storage::disk('s3')->url('posters/');
        $get1 = DB::table('tops')
            ->selectRaw('t_name AS name, t_poster AS poster, tops.created_at,tops.series_id AS id')
            ->join('series', 'series.t_id', '=', 'tops.series_id');
        $getTops = DB::table('tops')
            ->selectRaw('m_name AS name, m_poster AS poster, tops.created_at,tops.movie_id AS id')
