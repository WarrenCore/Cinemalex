<template>
  <div>
    ...
    <div class="my-2 p-3">
      <div class="btn-group">
        <button type="button" class="btn btn-sm btn-outline-primary" data-toggle="modal" data-target="#CreateActorModal">
          Create new actor
        </button>
      </div>
      <hr>
        ...
        <div class="modal-dialog" role="document">
          <div class="modal-content">
              ...
              <div class="create-actor">
                ...
                <div class="form-group col-md-12">
                  <div class="image-upload">
                    <input type="file" id="photo" name="photo" v-validate="'required|image'" @change="readImage('photo','photoFileImage')" class="inputfile">
                    <label id="photorLabel" for="photo">
                      <i class="fa fa-cloud-upload" aria-hidden="true"></i>
                      <img src="" id="photoFileImage" width="100%" style="display: none;">
                    </label>
                  </div>
            ...
            <div class="modal-footer">
              <button v-if="!button_loading" type="button" class="btn btn-sm btn-outline-primary" @click="CREATE_ACTOR()">Create</button>
              <button v-if="button_loading" class="btn btn-sm btn-outline-primary" disabled>
        ...
        <div class="col-8 col-sm-3 col-lg-2 mt-2 actor" v-for="(item,index) in data.actors.data" :key="index">
          <div class="actor-image">
            ...
            <div class="image-upload" v-if="index === show_key">
              <img :src="data.cdn.cdn_cast+item.image" :id="'preview_'+item.id" width="100%" class="img-rounded">
            ...
            <div class="details">
              ...
              <button v-if="index === show_key && !button_loading" class="btn btn-sm btn-outline-primary" @click="UPDATE(item.id,index)">Update</button>
              <button v-if="button_loading === item.id" class="btn btn-sm btn-outline-primary" disabled>
                <i class="fa fa-circle-o-notch fa-spin"></i> Loading</button>
              <p v-if="index !== show_key">{{item.name}}</p>
            </div>
          </div>
